...
class CW_InputParser {
  ...
  init(callback) {
    ...
    try {
      ...
      fileSystem.readFile(this.mPath + this.mFileName, "utf8", function (error, data) {
        if (error) {
          // Pass missing file errors to the caller
          passedThis.mInputString = JSON.stringify({
            "error": "Make sure that this file exists: " + error.path
          });
  ...
  static makeRunnerObjects({
    ...
    return new Promise((resolve, reject) => {
      resolver.resolve_makeRunnerObjects(resolve, reject, {
        domain: domain,
        directory: directory
      });
    });
