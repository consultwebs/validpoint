...
class CW_Runner {
  ...
  runCommand({
    ...
    let connectionPort = 80; // Rejections have been resolved before getting back to here, so we only return resolved Promises

    switch (command) {
      case "all-website-content":
        return new Promise((resolve, reject) => {
          this.command_AllWebsiteContent({
      ...
      case "https-port":
        ...
        return new Promise((resolve, reject) => {
          this.command_WebsiteAvailability({
            configObject: configObject,
            adviceObject: adviceObject,
            port: connectionPort
          }).then(result => {
  ...
  command_SSL({
    ...
    return new Promise((resolve, reject) => {
      let AdviceContent = require("./CW_AdviceContent"); // AdviceContent.progressContent( { configObject: configObject,
      // 	input: "Retrieving SSL certificate information for ".header + configObject.url.subject + "...   ".header 
      // });


      CW_Runner.network.checkSSL({
        ...
        AdviceContent.progressAdvice({
          configObject: configObject,
          adviceObject: adviceObject,
          testKey: "SSL"
        });
        adviceObject.test_result.results.push(adviceObject.item_result);
        adviceObject.finalizeOutput({
          stripConfigObject: false,
          stripItemResult: true
        });
        CW_Runner.network.checkSSLExpiration({
          url: configObject.url
        }).then(result => {
          ...
          AdviceContent.progressAdvice({
            configObject: configObject,
            adviceObject: adviceObject,
            testKey: "SSL_EXPIRATION"
          });
          adviceObject.test_result.results.push(adviceObject.item_result);
          adviceObject.finalizeOutput({
            stripConfigObject: true,
            stripItemResult: true
          });
  ...
  command_AllWebsiteContent({
    configObject = null,
    adviceObject = null
  }) {
    return new Promise((resolve, reject) => {
      CW_Runner.network.checkWebsiteContent({
        url: configObject.url
      }).then(result => {
        resolve(result);
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        CW_Runner.network.checkWebsiteContent({
          ...
          let root = HtmlParser.parse(result); // Stuff some nodes into an object for testing
          ...
          adviceObject.test_result.results.forEach(result => {
            result.raw_response = "";
          });
          resolve(JSON.stringify(adviceObject));
        }).catch(error => {
  ...
  command_LocalDns({
    ...
    return new Promise((resolve, reject) => {
      ...
      CW_Runner.network.checkLocalDns().then(result => {
        AdviceContent.progressContent({
          configObject: configObject,
  ...
  command_LocalNetwork({
    ...
    return new Promise((resolve, reject) => {
      ...
      CW_Runner.network.checkLocalNetwork().then(result => {
        AdviceContent.progressContent({
          configObject: configObject,
  ...
  command_Domain({
    ...
    let AdviceContent = require("./CW_AdviceContent.js"); // AdviceContent.progressTitle( { configObject: configObject, input: "Beginning Domain tests...   \n" } );
    ...
    adviceObject.domainResponses = {
      ...
      servers: {
        ...
        tld_cname: [],
        // list of cname records for @
        www_cname: [],
        // list of cname records for www
        mx: [],
        // list of Mail eXchange servers
        tld_a: [],
        // list of A records for the domain
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        async.waterfall([completion => {
          ...
          CW_Runner.network.checkDomain({
            domain: configObject.domain,
            recordType: "NS",
            queryServer: null
          }).then(result => {
            result.forEach(resultItem => {
              adviceObject.domainResponses.servers.ns.push(StringUtil.stripTrailingDot(resultItem.value));
            });
            AdviceContent.progressAdvice({
              configObject: configObject,
              adviceObject: adviceObject,
        ...
        }, (result, completion) => // Step 2. Parse the initial response and perform a dig query against an authoritative name server to get complete MX records fot the TLD
        {
          ...
          CW_Runner.network.checkDomain({
            ...
            result.forEach(resultItem => {
              adviceObject.domainResponses.servers.mx.push(StringUtil.stripTrailingDot(resultItem.value));
            });
            AdviceContent.progressAdvice({
              configObject: configObject,
              adviceObject: adviceObject,
        ...
        }, (result, completion) => // Step 3. Perform a dig query against an authoritative name server to get an A record for the domain (should be the @ record)
        {
          ...
          CW_Runner.network.checkDomain({
            domain: configObject.domain,
            recordType: "A",
            queryServer: result.domainResponses.servers.ns[0]
          }).then(result => {
      ...
      } catch (error) {// Each of the blocks above have their own catch, so this would only kick in if 
      ...
      async.waterfall([completion => {
        CW_Runner.network.checkWebsiteAvailability({
          ...
          try {
            ...
            if (adviceObject.item_result.result == CW_Constants.RESULT_PASS || adviceObject.item_result.result == CW_Constants.RESULT_UNTESTED) {
              completion(null, adviceObject);
  ...
  static getYargs() {
    ...
    yargs.scriptName("validpoint").usage("USAGE: $0 <command> -d [domain1,[domain2,...]] [-f file] [-r] [-q] [-h]").version("0.0.1").option("d", {
      alias: "domain",
  ...
  static processinputArguments() {
    return new Promise((resolve, reject) => {
      ...
      yargs.argv; // Pull a file name from the command line or use the default "validpoint.json"
      ...
      if (usefile && usefile.length > 0) {
        ...
        parser.init(function () // init callback
        {
