...
class CW_AdviceContent_Website extends CW_AdviceContent {
  ...
  advise() {
    ...
    if (this.test_result.result == CW_Constants.RESULT_FAIL && this.test_result.raw_response.message && this.test_result.raw_response.message.length > 0) {
      ...
      if (this.command == "website-content") {
        let tags = this.resultTagsForContent({
          inputObject: this.test_result.raw_response
        }); // Nothing was wrong

        if (tags.length < 1) {
          this.severity = this.resultTagToSeverity({
            resultTag: CW_Constants.RESULT_PASS
          });
          ...
          this.content = "";
          tags.forEach(tag => {
            let severity = this.resultTagToSeverity({
              resultTag: tag.result_value,
              extraKey: tag.intermediate_key
            });
            ...
            this.content += this.contentForSeverity({
              severity: severity,
              extraInput: tag.intermediate_key
            }) + "\n";
            this.result = tag.result_value;
          });
        }
      } else if ((this.command == "http-response" || this.command == "https-response" || this.command == "website" || this.command == "secure-website") && (this.test_result.raw_response.raw_response == "NO_RESPONSE" || // raw_response for http(s)-response generated objects is an object containing a raw_response
      this.test_result.raw_response == "NOT_FOUND" || this.test_result.raw_response == "TIMED_OUT")) {
      ...
      } else if (this.command == "website" || this.command == "secure-website") // website command passed useful information from the test back to here
        {
          ...
          this.severity = this.resultTagToSeverity({
            resultTag: this.test_result.result,
            extraKey: "REPLACEMENT"
          });
          this.content = this.contentForSeverity({
            severity: this.severity,
            extraInput: "REPLACEMENT"
          });
  ...
  contentForSeverity({
    ...
    switch (severity) {
      case CW_Constants.SEVERITY_NOTICE:
        if (extraInput && undefined != strings[this.command][extraInput][CW_Constants.NAME_SEVERITY_NOTICE]) {
      ...
      case CW_Constants.SEVERITY_URGENT:
        if (extraInput && undefined != strings[this.command][extraInput][CW_Constants.NAME_SEVERITY_URGENT]) {
  ...
  resultTagsForContent({
    inputObject = null
  }) {
    let returnValue = []; // We get a DOM tree if there are real results to examine. Otherwise, there was an error getting the DOM
    ...
    if (!inputObject.headNode.childNodes || inputObject.headNode.childNodes.length < 1) {
      returnValue.push({
    ...
    if (!inputObject.titleNode.childNodes || inputObject.titleNode.childNodes.length < 1 || inputObject.titleNode.rawText.length < 1) {
      returnValue.push({
    ...
    if (!inputObject.bodyNode.childNodes || inputObject.bodyNode.childNodes.length < 1 || inputObject.bodyNode.rawText.length < 1) {
      returnValue.push({
    ...
    if (!inputObject.h1Node.childNodes || inputObject.h1Node.childNodes.length < 1
    /*|| // requires separate parsing if there's more than one
    ...
    if (inputObject.metaNodes && inputObject.metaNodes.length > 0) {
      inputObject.metaNodes.forEach(node => {
  ...
  inProgressAdvice({
      ...
      switch (testKey) {
        ...
        case "SSL_EXPIRATION":
          if (this.test_result.result == CW_Constants.RESULT_FAIL) {
            ...
            severity = this.resultTagToSeverity({
              resultTag: CW_Constants.RESULT_PUNT
            });
            let content = this.contentForSeverity({
              severity: severity,
              extraInput: "EXPIRE_SOON"
            });
            process.stdout.write(content.warn + "\n");
          } else {
            process.stdout.write("expired\n".error);
            severity = this.resultTagToSeverity({
              resultTag: CW_Constants.RESULT_FAIL
            });
            let content = this.contentForSeverity({
              severity: severity,
              extraInput: "EXPIRED"
        ...
        case "WEBSITE_CONTENT":
          ...
          if (typeof this.test_result.raw_response !== "object") {
            ...
            let content = ""; // These can all be done at once, but are separated here to show "progress"

            process.stdout.write("Looking for HTML \"HEAD\" tag...   ".header);

            if (!this.test_result.raw_response.headNode.childNodes || this.test_result.raw_response.headNode.childNodes.length < 1) {
              process.stdout.write("failed\n".error);
              severity = this.resultTagToSeverity({
                resultTag: CW_Constants.RESULT_FAIL
              });
              content = this.contentForSeverity({
                severity: severity,
                extraInput: "HEAD_NONE"
              });
              process.stdout.write(content.error + "\n");
            } else {
              process.stdout.write("good\n".ok);
            ...
            if (!this.test_result.raw_response.bodyNode.childNodes || this.test_result.raw_response.bodyNode.childNodes.length < 1 || this.test_result.raw_response.bodyNode.rawText.length < 1) {
              process.stdout.write("failed\n".error);
              severity = this.resultTagToSeverity({
                resultTag: CW_Constants.RESULT_FAIL
              });
            ...
            if (!this.test_result.raw_response.titleNode.childNodes || this.test_result.raw_response.titleNode.childNodes.length < 1 || this.test_result.raw_response.titleNode.rawText.length < 1) {
              process.stdout.write("failed\n".error);
              severity = this.resultTagToSeverity({
                resultTag: CW_Constants.RESULT_FAIL
              });
            ...
            if (!this.test_result.raw_response.h1Node.childNodes || this.test_result.raw_response.h1Node.childNodes.length < 1) {
              process.stdout.write("warning\n".warn);
              severity = this.resultTagToSeverity({
                resultTag: CW_Constants.RESULT_PUNT
              });
            ...
            if (this.test_result.raw_response.metaNodes && this.test_result.raw_response.metaNodes.length > 0) {
              this.test_result.raw_response.metaNodes.forEach(node => {
                if (node.rawAttrs.indexOf("noindex") > -1) {
                  foundNoindex = true;
                }
              });
        ...
        case "WEBSITE_AVAILABILITY":
          ...
          if (parseFloat(extraInput.raw_response.avg) + "" !== "NaN") {
            ...
            if (extraInput.raw_response.avg > CW_PromiseResolver.MAX_HTTTP_RESPONSE_TIME) {
              ...
              process.stdout.write(this.contentForSeverity({
                severity: CW_Constants.SEVERITY_NOTICE,
                extraInput: "TIMED_OUT"
              }).warn + "\n");
            } else {
    ...
    if (returnType == "screen") {// process.stdout.write( returnValue.printAnswer + "\n" );
