...
class CW_AdviceContent_WebsiteAdmin extends CW_AdviceContent {
  ...
  inProgressAdvice({
    ...
    switch (testKey) {
      ...
      case "NS":
        // Show a header
        returnValue.printSubject = "Found name servers for ".text + configObject.domain.subject + ": ".text; // Fill an array with results to display

        serverArray = this.test_result.servers.ns; // Push errors or notices to the `tags` array
    ...
    if (tags.length < 1) {
      ...
      returnValue.result = CW_Constants.RESULT_PASS; // returnValue.printAnswer = "good".ok;
  ...
  advise() {
    let tags = this.resultTagsForDomain({
    ...
    if (tags.length < 1) {
      if (this.command == "domain") {
      ...
      } else // http(s)-port commands
        {
          ...
          if (this.test_result.result == CW_Constants.RESULT_PUNT) {// TODO: Get config to see if we should notify
          }

          let extraKey = null;

          if ((this.command == "http-port" || this.command == "https-port") && this.test_result.raw_response.raw_response == "NO_RESPONSE") {
  ...
  contentForSeverity({
    ...
    switch (severity) {
      case CW_Constants.SEVERITY_OK:
        if (extraInput && undefined != strings[this.command][extraInput][CW_Constants.NAME_SEVERITY_OK]) {
