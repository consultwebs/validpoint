...
if(response.statusCode.toString().startsWith("5"))// 500-level errors
{
returnValue.result=CW_Constants.RESULT_FAIL;
}else
if(response.statusCode.toString().startsWith("3"))// Redirects
{
...
if(response.statusCode!=200)// Everything else that's not '200'
{
returnValue.result=CW_Constants.RESULT_FAIL;
}
...
if(error.reason)// If there's a failure reason that isn't contained in a predictable tag
{
...
// There was no response from the website
{
returnValue=
{
result:CW_Constants.RESULT_FAIL,
...
// Not sure this can be triggered since providing bad URLs and ports results in resolvable errors above
{
returnValue=
{
result:CW_Constants.RESULT_FAIL,
...
// input is a simple string - assumed one domain
{
let runObject={
"domain":domain.trim(),
...
// request all domains TODO: Update this
{
let fs=require("fs");
let path="./";
