...
class CW_PromiseResolver {
  ...
  resolve_localNetwork(resolve, reject) {
    try {
      ...
      let resolved = false;
      PING_HOSTS.forEach((pingHost, index) => {
        // Perform a ping to prove the default status
        ping.sys.probe(pingHost, isAlive => {
          if (isAlive && !resolved) {
  ...
  resolve_checkLocalDns(resolve, reject) {
    try {
      ...
      dns.resolve4(DNS_HOST, (error, addresses) => {
        ...
        resolve(msg); // No need to reject() since all errors put us in a resolvable failure state
      });
    } catch (error) {
      let returnError = { ...new Error(),
        raw_response: error,
  ...
  resolve_checkSSL(resolve, reject, {
    url = null
  }) {
    // TODO: try/catch
    ...
    ssllabs.scan(options, (error, host) => {
      if (error) {// TODO: Do something. invalid certificates do not trigger this error
      } else {
        // iterate endpoints
  ...
  resolve_checkSSLExpiration(resolve, reject, {
    url = null
  }) {
    // TODO: try/catch
    let returnValue = {
  ...
  async resolve_checkWebsiteContent(resolve, reject, {
    url = null
  }) {
    try {
      ...
      let outerHtml = await page.evaluate(() => {
        let html = document.querySelector("html").outerHTML; // There's no HTML node
  ...
  resolve_checkWebsiteResponse(resolve, reject, {
    url = null,
    port = 80
  }) {
    try {
      ...
      const request = protocol.request(options, response => {
        ...
        response.on("end", () => {
          ...
          returnValue = {
            ...
            response_time: -1,
            raw_response: response.statusCode,
            // send the status code as the raw response
  ...
  resolve_checkWebsiteAvailability(resolve, reject, {
    domain = null,
    port = 80
  }) {
    try {
      ...
      let returnValue; // Ping configuration parameters
      ...
      tcpPing.ping(tcpConfig, (error, response) => {
        if (!error) // Default is "down" so there's nothing to change for an error response
          {
  ...
  resolve_checkDomain(resolve, reject, {
    domain = null,
    recordType = null,
    queryServer = null
  }) {
    try {
      ...
      dig([domain, recordType]).then(result => {
        ...
        if ((recordType == "NS" || recordType == "A") && (!result.answer || result.answer.length < 1)) {
          reject("NO_ANSWER");
  ...
  resolve_makeRunnerObjects(resolve, reject, {
    domain = null,
    directory = null
  }) {
    let returnValue = []; // Parse `domain` from command line input
    ...
    if (domain && domain != "all") {
    ...
    } else // request all domains TODO: Update this
      {
        ...
        let path = "./"; // Iterate all files in the input directory
