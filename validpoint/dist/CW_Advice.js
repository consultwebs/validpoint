...
class CW_Advice {
  ...
  constructor() {
    ...
    this.test_result = {
      "results": [],
      // Results of individual tests to iterate and parse when all tests are complete for a given domain.
  ...
  finalizeOutput({
    ...
    this.test_result.results.forEach(result => {
      // Push the parsed result to the 'actions' 
      let parsedResult = this.parseResult({
        result: result
  ...
  parseResult() {
    ...
    if (returnValue.result == CW_Constants.RESULT_PASS) {
      returnValue.severity = CW_Constants.SEVERITY_OK; // Set "OK" at the top-level if we've only seen "IGNORE" 

      this.greatest_severity = CW_Constants.SEVERITY_OK > this.greatest_severity ? CW_Constants.SEVERITY_OK : this.greatest_severity;
      ...
      {
        let adviceContent; // Use separate handlers for each command category	
        ...
        returnValue.result = adviceContent.result; // If this is the greatest severity we've seen, set the new top-level

        this.greatest_severity = adviceContent.severity > this.greatest_severity ? adviceContent.severity : this.greatest_severity;
